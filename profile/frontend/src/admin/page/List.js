...
const List = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5000/page?key=" + search, {
        headers: {
  ...
  const handleDeleteItem = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("http://localhost:5000/page/delete/" + id, {
        method: "DELETE",
        headers: {
          Authorization: "Bearer " + token,
        },
    ...
    <>
      ...
      <div className={"drawer " + (isSidebarOpen ? "lg:drawer-open" : "")}>
        ...
        <div className="drawer-content flex flex-col">
          {/*----------------------------------------------*/}
          <Breadcrumb pathLocation={[{ myroute: "page_list", name: "Page" }]} />
